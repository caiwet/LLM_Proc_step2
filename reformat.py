...
INPUT_DIR = '/data/bwh-comppath-img2/MGH_CID/LLM_Proc/step2_gemini_output/final.parquet'
OUTPUT_DIR = '/data/bwh-comppath-img2/MGH_CID/LLM_Proc/step2_processed_output'
...
if __name__ == "__main__":
    ...
    # batch_ids = [10, 100, 500, 1000, 5000, 10000]
    # df = df[df['batch_id'].isin(batch_ids)].reset_index(drop=True)
    ...
    for idx, row in df.iterrows():
        try:
            ...
            print(f"\nProcessing batch_id: {batch_id} (row {idx+1}/{len(df)})")
            ...
            flat_out = os.path.join(OUTPUT_DIR, f"{batch_id}_codes.parquet")
            ...
            refs_out = os.path.join(OUTPUT_DIR, f"{batch_id}_refs.parquet")
