...
import traceback
def load_note_time(row):
    try:
        note_time = datetime.fromisoformat(row["note_time"])
        date_shift = timedelta(days=row["date_shift"])
        shifted_time = note_time + date_shift
        return shifted_time.strftime("%Y-%m-%d")
    except ValueError:
        return "unknown"

def build_prompt(row: pd.Series) -> str:
    instruction = 'Extract concepts from the following clinical note.\n\n'
    note_date = f'Note date: {row['shifted_note_time']}.\n'
    note_text = row['transformed_text']
...
class VLLMParquetProcessor:
    def __init__(self, num_servers=8, base_port=8000, max_concurrent=256, host="0.0.0.0"):
        ...
        # self.servers = [f"http://{host}:{base_port + i}/v1" for i in [7]]
        ...
        async with semaphore:
            ...
            try:
                if last_max_tokens > 0:
                    max_tokens = min(int(last_max_tokens) * 2, 16384)
                else:
                    max_tokens = 2048
    ...
    async def process_file(self, 
        ...
        if prompt_column not in df.columns:
            ...
            # Shift dates
            df['shifted_note_time'] = df.apply(load_note_time, axis=1)
